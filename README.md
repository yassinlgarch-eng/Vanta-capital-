...
#### معالجة الأخبار قبل العرض

كل خبر يصل من المزوّد يمرّ بخطوات إعداد قبل العرض للمستخدم العربي:

1. **فلترة زمنية:** يُستبعد أي خبر أقدم من 72 ساعة.
2. **استعلام مالي مُحكم:** نسأل المزوّدين عن مواضيع مثل *Fed, ECB, interest
   rate, inflation, S&P 500, oil, gold, Bitcoin, forex, treasury yield…* فقط.
3. **مُصنّف مالي محلّي:** ملف `src/lib/translate.ts` يطابق العنوان الإنجليزي
   مع قائمة قواعد مالية. أي خبر لا يطابق أي قاعدة (مثلاً سياسة عامة بلا أثر
   مالي مباشر) يُحذف من القائمة.
4. **عَرَبَنَة العنوان والملخص:** القاعدة المطابقة تُولّد عنواناً وملخّصاً عربياً
   احترافياً، ويُحفظ العنوان الإنجليزي الأصلي في حقل `originalTitle` ويُعرض
   تحت العنوان العربي بخط أصغر للحفاظ على المصدر والشفافية.
5. **حالة "لا توجد أخبار حديثة":** إذا نجح الاتصال لكن لم تنجح أي قاعدة في
   التطابق، يُعرض للمستخدم: *«لا توجد أخبار حديثة متاحة من مزود الأخبار حاليًا.»*
6. **زر "اقرأ المزيد":** يفتح Modal احترافي RTL يحوي رابط المصدر الأصلي
   (Reuters / Bloomberg / CNBC…) في تبويب جديد + إخلاء مسؤولية.
